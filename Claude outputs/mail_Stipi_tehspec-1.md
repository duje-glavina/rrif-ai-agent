...
Dvije zamjene: hunspell rječnik (traži superusera i pristup datotečnom sustavu)
ili normalizacija u Pythonu u zaseban stupac. Testirao sam drugu — `chunk_text_stem`
nad cijelim korpusom, GIN indeks, isto pitanje kroz isti normalizator.

**Rezultat je nula.** Pet pitanja se pomakne, neto oko ništa: jedno dobije,
jedno izgubi, ostatak se prešeta među metrikama. Ostaje `simple` konfiguracija —
bez hunspella, bez lematizacije u ingestu. Vraćam se na to tek ako validacijski
set RRiF-ovih savjetnika pokaže promašaje koji izgledaju morfološki.

**3. Baza: samohostano — zatvaram to pitanje**

Nema postotka dostupnosti (tvoj mail), baza je ~1,5 GB na 10 godina građe.
Managed PG ne donosi ništa što nam treba, a dodaje podobrađivača i drugi DPA.
Ostaje na istom boxu, Unix socket.

Pošteno: jedan argument koji sam ranije navodio ovdje otpada. Tvrdio sam da nam
treba datotečni pristup zbog hunspella — nakon mjerenja iz točke 2 hunspell nam
ne treba, pa to više nije razlog. Ostatak stoji.

Sitno, i ispravak mene ranijeg: PG 16 iz tvoje scheme je u redu. Provjerio sam
strojeve — radna stanica na kojoj je korpus vrti 16.15 (Ubuntu 24.04 default),
laptop 17.9. Znači ne razilaze se schema i baza, nego moja dva stroja međusobno.
Ništa što koristimo ne traži 17, pa predlažem da 16 bude ciljana verzija i da to
zapišemo — inače na Hetzneru dobijemo 16 iz Ubuntuovih repozitorija, a netko drugi
kasnije doda PGDG i dobije 17.
